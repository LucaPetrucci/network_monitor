...
run_case_120s() {
  ...
  local run_start
  run_start=$(date '+%F %T')
  echo "[ROUND5][$label] START mode=$mode packet_size=$psize @ $(date -u '+%T')"

  sudo bash -lc "nohup bash -c 'printf \"\\n\" | network_monitor -i vethA -t 10.10.0.2 -S 10.20.0.1 -I vethB -p 5050 -m $mode -l $psize' >/tmp/nm_round5_${label}.log 2>&1 & echo \$! >/tmp/nm_round5_${label}.pid"

  # total 120s = 45 + 12(outage) + 63
  sleep 45
  echo "[ROUND5][$label] OUTAGE down/up 12s @ $(date -u '+%T')"
  sudo ip netns exec nsRemote ip link set dev vethA-ns down
  sleep 12
  sudo ip netns exec nsRemote ip link set dev vethA-ns up
  sleep 63

  if [[ -f /tmp/nm_round5_${label}.pid ]]; then
    sudo kill "$(cat /tmp/nm_round5_${label}.pid)" 2>/dev/null || true
  fi
  ...
  echo "[ROUND5][$label] END @ $(date -u '+%T') local_samples=$local_samples remote_samples=$remote_samples"
}
...
summary() {
  echo "[ROUND5] local grouped summary (last 4h)"
  ...
  echo "[ROUND5] remote_sim grouped summary (last 4h)"
  mysql -u myuser -p'mypassword' network_monitor_remote_sim -e "
    SELECT protocol, packet_size, LEFT(executed_command,90) AS cmd, COUNT(*) AS samples,
           ROUND(AVG(bitrate),2) AS avg_mbps, ROUND(MAX(lost_percentage),2) AS max_loss,
           MIN(timestamp) AS start_ts, MAX(timestamp) AS end_ts
  ...
  echo "[ROUND5] latest interruptions local"
  ...
  echo "[ROUND5] latest interruptions remote_sim"
...
main() {
  echo "[ROUND5] START UTC: $(date -u '+%F %T')"
  prepare
