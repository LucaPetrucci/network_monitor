...
run_case() {
  ...
  echo "[ROUND2][$label] START mode=$mode packet_size=$psize @ $(date -u '+%T')"
  local run_start
  run_start=$(date '+%F %T')

  sudo bash -lc "nohup bash -c 'printf \"\\n\" | network_monitor -i vethA -t 10.10.0.2 -S 10.20.0.1 -I vethB -p 5050 -m $mode -l $psize' >/tmp/nm_round2_${label}.log 2>&1 & echo \$! >/tmp/nm_round2_${label}.pid"

  sleep 20
  echo "[ROUND2][$label] OUTAGE down/up 12s @ $(date -u '+%T')"
  sudo ip netns exec nsRemote ip link set dev vethA-ns down
  sleep 12
  sudo ip netns exec nsRemote ip link set dev vethA-ns up
  sleep 28

  if [[ -f /tmp/nm_round2_${label}.pid ]]; then
    sudo kill "$(cat /tmp/nm_round2_${label}.pid)" 2>/dev/null || true
  fi
  ...
  echo "[ROUND2][$label] END + mirrored @ $(date -u '+%T')"
}

echo "[ROUND2] START UTC: $(date -u '+%F %T')"
...
curl -sS -u admin:admin -H 'Content-Type: application/json' -X PUT \
  ...
  }' >/tmp/remote_ds_update_round2.json
...
echo "[ROUND2] END UTC: $(date -u '+%F %T')"

echo "[ROUND2] local summary"
mysql -u myuser -p'mypassword' network_monitor -e "
  SELECT protocol, packet_size, COUNT(*) AS samples, ROUND(AVG(bitrate),2) AS avg_mbps, ROUND(MAX(lost_percentage),2) AS max_loss
  FROM iperf_results
  WHERE timestamp >= DATE_SUB(NOW(), INTERVAL 4 HOUR)
  GROUP BY protocol, packet_size
  ORDER BY protocol, packet_size;
"

echo "[ROUND2] remote_sim summary"
...
echo "[ROUND2] latest interruptions local"
mysql -u myuser -p'mypassword' network_monitor -e "SELECT id,timestamp,interruption_time FROM interruptions ORDER BY id DESC LIMIT 10;"
