...
echo "[NUC2] iface=$IFACE local=$LOCAL_IP target=$TARGET_IP port=$PORT"
echo "[NUC2] run_seconds=$RUN_SECONDS pause_seconds=$PAUSE_SECONDS"
echo "[NUC2] START_AT=$START_AT"

echo "[NUC2] Waiting for start time..."
while [[ "$(date +%H:%M:%S)" < "$START_AT" ]]; do
  sleep 0.2
done

echo "[NUC2] START $(date '+%F %T')"

for mode in udp tcp; do
  for size in 500 1000 1472; do
    echo "[NUC2] CASE mode=$mode size=$size begin $(date '+%T')"
    timeout "${RUN_SECONDS}s" bash -lc "printf '\n' | network_monitor2 -i $IFACE -t $TARGET_IP -S $LOCAL_IP -I $IFACE -p $PORT -m $mode -l $size" || true
    echo "[NUC2] CASE mode=$mode size=$size end   $(date '+%T')"
    sleep "$PAUSE_SECONDS"
  done
done

echo "[NUC2] DONE $(date '+%F %T')"
